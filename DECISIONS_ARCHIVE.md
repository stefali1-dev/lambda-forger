...
This file contains archived (historical/superseded/closed) decision context.
Active decisions and blockers live in `DECISIONS.md`.
...
### Decision 21: Brand direction = "AI Lambda Forger" (Forge identity)
**Date:** 2026-02-15  
...
2. Direction: Forge visual language (developer-tool feel, practical copy)
...
### Decision 23: Workspace UX hardening and Monaco diagnostics tuning
**Date:** 2026-02-15  
**Context:** Initial Workspace Mode implementation exposed UX friction and visual inconsistencies (prompt/confirm flows, contrast issues, file picker interaction bugs, and noisy Monaco diagnostics).

**Chosen:**
1. Replace `window.prompt/window.confirm` flows with shadcn `Dialog` and `AlertDialog`
2. Remove ambiguous `*` tab marker and show explicit `entry` label for deploy target
3. Rework file upload trigger to explicit hidden input + ref click (no nested interactive elements)
4. Move workspace styling to explicit component-level classes for cards/buttons/inputs
5. Tune Monaco TypeScript diagnostics to keep syntax checks while reducing false semantic noise for template context

**Why:**
- Improves clarity and consistency for file-management workflows
- Fixes interaction reliability for file selection in both normal and workspace contexts
- Eliminates confusing visual artifacts in dark workspace panel surfaces
- Keeps editor useful by surfacing syntax issues without irrelevant type-environment errors

**Trade-offs:**
- Additional UI state for dialog orchestration
- Monaco semantic diagnostics are intentionally relaxed in-editor (runtime validation still handled by backend deploy checks)

---

### Decision 24: Naming normalization to `forger` for active runtime/service identifiers
**Date:** 2026-02-15  
**Context:** Product/docs branding is `AI Lambda Forger`, while some active backend identifiers still used `builder` names.

**Chosen:**
1. Use `forger` naming for active runtime identifiers (`service` health value, SAM description, default execution role name)
2. Keep env override support (`MVP_LAMBDA_ROLE_NAME`) so existing setups can continue without migration blockers
3. Treat historical `builder` names in this file/logs as legacy context only

**Why:**
- Reduces cognitive overhead when debugging and operating the stack
- Keeps docs, UI messaging, and runtime identifiers aligned
- Avoids forced migrations by preserving override-based compatibility

**Trade-offs:**
- Existing deployments may temporarily have mixed legacy/new resource names
- Some historical entries in `DECISIONS.md` intentionally remain unchanged

---

### Decision 25: Documentation hygiene = periodic pruning of stale/repetitive content
**Date:** 2026-02-15  
**Context:** Guides (`AGENTS.md`, `PLAN.md`) accumulated repetitive or outdated details, increasing token usage and reducing signal.

**Chosen:**
1. Instruct agents to periodically prune stale/redundant sections during normal workflow
2. Keep canonical, high-value guidance: current scope, contracts, runbook, decisions, blockers
3. Move historical narrative details to concise progress notes only when they affect current decisions

**Why:**
- Keeps agent context efficient and cheaper to process
- Reduces drift risk between implementation and docs
- Improves onboarding speed for both humans and LLM agents

**Trade-offs:**
- Requires discipline to avoid over-pruning important historical rationale
- Occasional review needed to ensure critical edge-case notes are preserved
...
5. Remove or condense obsolete entries when they no longer help current decisions
...
**Last Updated:** 2026-02-15 23:10 EET
